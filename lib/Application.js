...
const {FeedReader} = require("Feeds");
    ...
    onAttach: worker => {
      ...
      worker.port.on("gridReady", _ => {
        ...
        for (var i=0; i < links.length; i++) {
          var link = links[i];
          link.imageURI = link.imageURI || PageThumbs.getThumbnailURL(link.url);

          FeedReader.feedFromLink(link).then(data => {
            if (data != null) {
              worker.port.emit("feedData", data);
            }
          },
          _ => {
            console.error("failed to obtain feed data for: " + link.url);
          });
