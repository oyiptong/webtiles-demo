...
let NewNewTab = {
  factory: {
    ...
    Component: Class({
      ...
      newChannel: function(uri) {
        let chan = Services.io.newChannel(data.url("html/newnewtab.html"), null, null);
  ...
  page: {
    contentScriptFile: [
      data.url("vendor/react-0.13.3/react-with-addons.min.js"),
      data.url("vendor/react-0.13.3/JSXTransformer.js"),
      data.url("js/newnewtab.js"),
    ],
    contentScriptWhen: 'ready',
    include: ["about:newnewtab"],
    onAttach: function(worker) {
      worker.port.emit("style", data.url("vendor/bootstrap-3.3.4-dist/css/bootstrap.min.css"));
      worker.port.emit("style", data.url("vendor/bootstrap-3.3.4-dist/css/bootstrap-theme.min.css"));
      worker.port.emit("style", data.url("css/newnewtab.css"));
    }
  }
}
